        ...
        if is_verified:
            if mfa_enabled:
                ...
                access_token = f_jwt.create_access_token(
                    ...
                    additional_claims={"user_type": user_type + "_temp"},
                    fresh=True,
...
class RefreshToken(Resource):
    ...
    def post(self):
        ...
        new_token = f_jwt.create_access_token(
            identity=curent_user_id,
            additional_claims={"user_type": current_user_type},
            fresh=False,
        )
...
class VerifyEmail(Resource):
    def get(self):
        ...
        GET_USER = "SELECT id, is_verified FROM {} WHERE email= %s".format(table_name)
        try:
            cursor = app_globals.get_named_tuple_cursor()
        ...
        if is_verified:
            ...
            UPDATE_USER_VERIFIED = (
                "UPDATE {} SET is_verified= %s, verified_at= %s WHERE id= %s".format(
                    table_name
                )
            )
            try:
