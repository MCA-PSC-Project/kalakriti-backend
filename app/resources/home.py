...
class ViewedProducts(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            JOIN product_base_item pbi ON p.id = pbi.product_id
            WHERE p.id IN (
                SELECT product_id FROM viewed_products WHERE customer_id = %s ORDER BY added_at LIMIT %s 
    ...
    def put(self):
        ...
        data = request.get_json()
        viewed_product_ids_list = json.loads(json.dumps(data))
        current_time = datetime.now()
        try:
            cursor = app_globals.get_cursor()
            UPSERT_VIEWED_PRODUCTS = """INSERT INTO viewed_products(customer_id, product_id, added_at) 
            VALUES(%s, %s, %s) 
            ON CONFLICT (customer_id, product_id) 
            DO UPDATE SET updated_at = %s"""
            values_tuple_list = []
            for viewed_product_id in viewed_product_ids_list:
                values_tuple = (
                    customer_id,
                    viewed_product_id,
                    current_time,
            ...
            psycopg2.extras.execute_batch(
                cursor, UPSERT_VIEWED_PRODUCTS, values_tuple_list
            )
        except (Exception, psycopg2.Error) as err:
            app.logger.debug(err)
            abort(400, "Bad Request")
            ...
            cursor.close()
        return f"viewed products updated sucessfully", 201
