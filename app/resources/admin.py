...
class CustomersInfo(Resource):
    ...
    def patch(self, customer_id):
        ...
        if user_type != "admin" and user_type != "super_admin":
            abort(
                403,
                "Forbidden: only super-admins and admins can enable/disable customer",
            )
        ...
        UPDATE_CUSTOMER_ENABLED_STATUS = (
            """UPDATE customers SET enabled= %s, updated_at= %s where id= %s"""
        )
        try:
    ...
    def delete(self, customer_id):
        ...
        if user_type != "admin" and user_type != "super_admin":
            abort(
                403,
                "Forbidden: only super-admins and admins can delete customer account",
            )
...
class SellersInfo(Resource):
    ...
    def patch(self, seller_id):
        ...
        UPDATE_SELLER_ENABLED_STATUS = (
            """UPDATE sellers SET enabled= %s, updated_at= %s where id= %s"""
        )
        try:
            cursor = app_globals.get_cursor()
            cursor.execute(
    ...
    def delete(self, seller_id):
        ...
        if user_type != "admin" and user_type != "super_admin":
            abort(
                403, "Forbidden: only super-admins and admins can delete seller account"
            )
