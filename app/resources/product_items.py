...
class SellersProductItems(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            cursor.execute(
                CREATE_PRODUCT_ITEM,
                (
                    product_item_dict.get("product_id"),
        ...
        app_globals.db_conn.autocommit = True
        return (
            f"product_item_id = {product_item_id} created in product_id = {product_item_dict.get('product_id')} successfully",
            201,
        )
        ...
        try:
            ...
            for row in rows:
                ...
                for row in rows:
                    ...
                    product_item_dict.update(
                        json.loads(json.dumps({"original_price": row[4]}, default=str))
                    )
                    product_item_dict.update(
                        json.loads(json.dumps({"offer_price": row[5]}, default=str))
                    )

                    product_item_dict["quantity_in_stock"] = row[6]
                    product_item_dict.update(
                        json.loads(json.dumps({"added_at": row[7]}, default=str))
                    )
                    product_item_dict.update(
                        json.loads(json.dumps({"updated_at": row[8]}, default=str))
                    )
    ...
    def put(self, product_item_id):
        ...
        try:
            ...
            cursor.execute(GET_VARIANT_VALUE_ID, (product_item_id,))
            row = cursor.fetchone()
            if not row:
        ...
        if "quantity_in_stock" in data.keys():
            ...
            try:
                ...
                DELETE_ITEM_OLD_MEDIAS = (
                    """DELETE FROM product_item_medias WHERE product_item_id = %s"""
                )
            ...
            if (
                ...
                abort(
                    400,
                    "only sellers, super-admins and admins can trash a product item",
                )
...
class SellersProductBaseItem(Resource):
    ...
    def patch(self):
        ...
        if (
            ...
            abort(
                400,
                "only seller, super-admins and admins can change base item of a product",
            )
        data = request.get_json()
        ...
        try:
            cursor = app_globals.get_cursor()
            CHECK_ITEM_EXISTS = (
                """SELECT true FROM product_items WHERE id = %s AND product_id = %s"""
            )
            cursor.execute(
                CHECK_ITEM_EXISTS,
                (
